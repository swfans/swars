...
short LbDataFree(struct TbLoadFiles *load_file)
{
    ...
    if (data != NULL) {
        if ((*data) != NULL) {
            BFLIB_DEBUGLOG(0,"freeing \"%s\"...",load_file->FName);
            LbMemoryFree(*data);
            (*data) = NULL;
            BFLIB_DEBUGLOG(0,"done");
        }
    }
    return 1;
}
...
short LbDataLoad(struct TbLoadFiles *load_file)
{
    ...
    if (fname[0] == '*')
    {
        BFLIB_DEBUGLOG(0,"%s: * in fname", fname);
    ...
    } else
    {
        ...
        BFLIB_DEBUGLOG(0,"%s: filelength %ld", fname, slength);
...
short LbDataLoadAll(struct TbLoadFiles load_files[])
{
    ...
    while (t_lfile->Start != NULL)
    {
        ...
        if ( ret_val == -100 ) {
            BFLIB_ERRORLOG("%s: Can't allocate memory", t_lfile->FName);
          ferror++;
        }
        else if ( ret_val == -101 ) {
            BFLIB_ERRORLOG("%s: Can't load file", t_lfile->FName);
          ferror++;
        }
        i++;
        t_lfile = &load_files[i];
