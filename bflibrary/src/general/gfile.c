...
i32 LbFileLengthRnc(const char *fname)
{
...
i32 LbFileLoadAt(const char *fname, void *buffer)
{
    i32 filelength = LbFileLengthRnc(fname);
...
i32 LbFileSaveAt(const char *fname, const void *buffer, u32 len)
{
    TbFileHandle handle;
    int result;
...
i32 LbFileStringSearch(TbFileHandle handle, const char *sstr, u8 *buf, u32 buflen, ushort flags)
{
    TbResult ret;
    i32 prevpos, sslen;
    char locbuf[132];
